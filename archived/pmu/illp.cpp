...
bool illp::pointInStdPalette( int x, int y )
{
	return (x >= paletteLeft && x <= paletteRight
		 && y >= paletteTop && y <= paletteBottom);
}

bool illp::pointInImage( int x, int y )
{
...
void illp::createGreyScaleImage()
{
	...
	unsigned int bytes_per_pixel = image->getPixelsRef().getBytesPerPixel();	// 3 ==> RGB or 4 ==> RGBA
...
void illp::addNewColorFromOriginalImage()
{
	// get the color of the pixel @ (selectedX, selectedY)
	unsigned char * pixels = image->getPixels();
	int bytes_per_pixel = image->getPixelsRef().getBytesPerPixel();
...
void illp::createPosterizedImage()
{
	...
	for (int i = 0; i < w; i += bytes_per_pixel){
		for (int j = 0; j < h; j++){
			if (image->getPixelsRef().getNumChannels() > 1)
			{
				int red = pixels[(j * w) + i];
				int green = pixels[(j * w) + i + 1];
				int blue = pixels[(j * w) + i + 2];
